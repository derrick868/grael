...
def place_order():
    ...
    if customer_cart:
        try:
            ...
            # create_order_response = service.collect.mpesa_stk_push(phone_number='0741174317', email=current_user.email,
            ...
            for item in customer_cart:
                ...
                new_order.payment_id = item.quantity
...
# Remove order route
@views.route('/remove-order/<int:order_id>', methods=['GET', 'POST'])
@login_required
def remove_order(order_id):
    order = Order.query.get(order_id)
    if order is None:
        flash("Order not found.", "danger")
        return redirect(url_for('views.orders'))  # Ensure you use 'views.orders' if it's part of the blueprint
    
    # Ensure the order belongs to the logged-in user
    if order.customer_link != current_user.id:
        flash("You don't have permission to delete this order.", "danger")
    ...
    flash("Your order has been successfully removed.", "success")
    return redirect(url_for('views.order'))  # Correct redirect after deletion
